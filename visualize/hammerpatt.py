...
class AggressorAccessPattern():
    ...
    def to_pandas_entry(self, max_period):
        ...
        return {"period": self.period, "frequency": int(max_period/self.period), "amplitude": self.amplitude, "phase": self.phase, "flips": True if self.flips else False, "label": label(self.aggr_tuple)}
...
class HammeringPattern():
    ...
    def from_json(cls, ddict):
        ...
        if not set(("id", "agg_access_patterns", "access_ids","address_mappings", "base_period", "max_period")).issubset( set(ddict.keys())): 
            return NotImplemented
        order = list(map(int, ddict['access_ids']))
        agg_list = list(map(AggressorAccessPattern.from_json, ddict['agg_access_patterns']))
        return cls(ddict['id'],  int(ddict['base_period']), int(ddict['max_period']),agg_list, order, ddict['address_mappings'])
...
class HammeringPatternInstance():
    ...
    def time_plot(self):
        ...
        fig, axes = plt.subplots(len(self.aggr_list), 1, sharex=True, constrained_layout=True)
    ...
    def freq_ampl_plot(self):
        ...
        ax2.set_xticklabels([self.max_period/x if x != 0 else "NaN" for x in ax2.get_xticks() ])
