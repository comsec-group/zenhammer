...
ExperimentConfig::ExperimentConfig(const std::string &filepath, size_t config_id) : filepath(filepath) {
  ...
  for (const auto &cfg : config["experiment_configs"]) {
    if (cfg["config_id"].as<size_t>() == config_id) {
      ...
      num_measurement_rounds = cfg["num_measurement_rounds"].as<size_t>();
...
ExperimentConfig::ExperimentConfig(execution_mode ExecMode,
    ...
    : config_id(0), exec_mode(ExecMode), num_measurement_rounds(NumMeasurementRounds), num_sync_rows(NumSyncRows),
      row_distance(RowDistance), row_origin_same_bg(RowOriginSameBg), row_origin_same_bk(RowOriginSameBk) {
}
