...
//    sweep_pattern(patt, mapper, hammering_num_reps, SWEEPING_SIZE);
//
...
size_t ReplayingHammerer::replay_patterns_brief(const std::string& json_filename,
                                                const std::unordered_set<std::string> &pattern_ids, size_t sweep_rows,
                                                bool running_on_original_dimm) {
  auto patterns = load_patterns_from_json(json_filename, pattern_ids);
  return replay_patterns_brief(patterns, sweep_rows, 1, running_on_original_dimm);
}
...
size_t ReplayingHammerer::replay_patterns_brief(std::vector<HammeringPattern> hammering_patterns,
  ...
  for (auto &pattern : hammering_patterns) {
    ...
    for (size_t i = 0; i < num_locations; ++i) {
      // do the sweep
      struct SweepSummary summary = sweep_pattern(pattern, mapper, 1, sweep_rows, direct_effective_aggs);
  ...
  filename << "sweep-summary-" << num_locations << "x" << sweep_rows << "rows.json";
...
size_t ReplayingHammerer::hammer_pattern(FuzzingParameterSet &fuzz_params, CodeJitter &code_jitter,
  ...
  // FIXME: Store the scheduling policy in the pattern JSON so we can recover it here.
  auto hammering_accesses_vec = mapper.export_pattern(pattern, program_args.scheduling_policy);
...
size_t ReplayingHammerer::hammer_pattern(FuzzingParameterSet &fuzz_params, CodeJitter &code_jitter,
  ...
  // FIXME: Store the fence type in the pattern JSON so we can recover it here.
  code_jitter.jit_strict(flushing_strategy, fencing_strategy, hammering_accesses_vec, program_args.fence_type, num_activations);
...
struct SweepSummary ReplayingHammerer::sweep_pattern(HammeringPattern &pattern, PatternAddressMapper &mapper,
  ...
  return sweep_pattern(pattern, mapper, num_reps, num_rows, {});
}
...
struct SweepSummary ReplayingHammerer::sweep_pattern(HammeringPattern &pattern, PatternAddressMapper &mapper,
  ...
  Logger::log_info(format_string("Sweeping pattern %s with mapping %s over %zu rows, with each %d repetitions.",
